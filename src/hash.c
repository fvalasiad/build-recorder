...
hash_file_contents(char *name, size_t sz)
{
    ...
    if (fd < 0) {
	fprintf(stderr, "hash.c:hash_file_contents():open(%s): %s\n", name,
		strerror(errno));
	return NULL;
    }
    ...
    if (buf == MAP_FAILED) {
	fprintf(stderr, "hash.c:hash_file_contents():mmap(%s): %s\n", name,
		strerror(errno));
	return NULL;
    }
    ...
    if (ret) {
	fprintf(stderr, "hash.c:hash_file_contents():madvise(%s): %s\n", name,
		strerror(errno));
    }
    ...
    if (hash == NULL) {
	fprintf(stderr, "hash.c:hash_file_contents():malloc(%s): %s\n", name,
		strerror(errno));
	return NULL;
    }
    ...
    if (munmap(buf, sz) < 0) {
	fprintf(stderr, "hash.c:hash_file_contents():munmap(%s): %s\n", name,
		strerror(errno));
	exit(EXIT_FAILURE);
    }
...
get_file_hash(char *fname)
{
    ...
    if (stat(fname, &fstat)) {
	fprintf(stderr, "hash.c:get_file_hash():stat(%s): %s\n", fname,
		strerror(errno));
	return NULL;
    }
